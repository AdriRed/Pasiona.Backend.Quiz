...
public static class ResultResource
{
    ...
    public static ResultResource<T> Errors<T>(params ErrorResource[] errors) => New((T)default).WithErrorResources(errors.AsEnumerable());

    public static ResultResource<T> Errors<T>(IEnumerable<ErrorResource> errors) => New((T)default).WithErrorResources(errors);
